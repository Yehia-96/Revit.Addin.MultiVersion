...
# Define server path for Revit 2026
$ServerDir2026 = Join-Path $UserProfile "Müller+Hereth GmbH\31 BIM - Dokumente\General\01 Software\01 Revit-Tools\Updates\Versions\2026"
...
Write-Host "Server 2026: $ServerDir2026"
...
# Copy updated version files to the 2026 server path
if (Test-Path $ServerDir2026) {
    Copy-Item $excelVersionFile -Destination $ServerDir2026 -Force
    Copy-Item $versionFile -Destination $ServerDir2026 -Force
    Write-Host "[OK] Copied version files to $ServerDir2026"
} else {
    Write-Host "[WARN] Server path not found, skipping: $ServerDir2026"
}

Write-Host "[OK] Version increment complete for Revit 2026."
