...
# Define server path for Revit 2027
$ServerDir2027 = Join-Path $UserProfile "Müller+Hereth GmbH\31 BIM - Dokumente\General\01 Software\01 Revit-Tools\Updates\Versions\2027"
...
Write-Host "Server 2027: $ServerDir2027"
...
# Copy updated version files to the 2027 server path
if (Test-Path $ServerDir2027) {
    Copy-Item $excelVersionFile -Destination $ServerDir2027 -Force
    Copy-Item $versionFile -Destination $ServerDir2027 -Force
    Write-Host "[OK] Copied version files to $ServerDir2027"
} else {
    Write-Host "[WARN] Server path not found, skipping: $ServerDir2027"
}

Write-Host "[OK] Version increment complete for Revit 2027."
